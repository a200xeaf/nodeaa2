        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (!pressedKeys.has(e.key) && midiKeyMap.has(e.key)) {
                ...
                if (midiNote !== undefined) {
                    ...
                    const midiMessage: Uint8Array = new Uint8Array([
                        144,
                        midiNote + data.midikeyboard_octave,
                        data.midikeyboard_velocity,
                    ]);
    ...
    return (
        <NodeaaContainer selected={selected} width={23} height={12} infoID="midiKeyboardNode">
            ...
            <div className="flex flex-col nodrag cursor-default bg-white p-2 h-[10rem] rounded-b-xl">
                ...
                <p>
                    Last key: {midiNumberToNote(lastKey[0])} with velocity{" "}
                    {lastKey[1] === undefined ? "nothing" : lastKey[1]}
                </p>
                <p>
                    Octave:{" "}
                    {data.midikeyboard_octave >= 0
                        ? `+${data.midikeyboard_octave / 12}`
                        : `${data.midikeyboard_octave / 12}`}
                </p>
                <PianoKeyboard
                    keyHeight={70}
                    keyWidth={39}
                    numKeys={16}
                    startNote={48 + data.midikeyboard_octave}
                    heldNotes={pressedMidiNotes}
                />
            </div>
            <Handle
                type="source"
